...
\item Está compuesto por: 
    \begin{itemize}
        \item Un \bld{estado}.                 \tikz[inlineBlock] \coordinate (propiedadesFrame)    ;
        \item Un \bld{comportamiento} definido.\tikz[inlineBlock] \coordinate (comportamientoFrame) ;
    \end{itemize}
\end{itemize}
...
\begin{tikzpicture}[overlay]
  ...
  \draw[->, thick] (propiedadesFig.west) -- ([xshift=3mm]propiedadesFrame.east);
  \node[miniconcept, below=3mm of propiedadesFig] (comportamientoFig) { Métodos };
  \draw[->, thick] (comportamientoFig.west) -- ([xshift=3mm]comportamientoFrame.east);
\end{tikzpicture}
...
\begin{tikzpicture}
    \umlclass[x=0,y=0]{Persona}%
      {--Nombre : String\\ --Edad : Integer\\ --IdiomaNativo : String\\ --Residencia : String}%
      {}
    \umlclass[x=6,y=0]{Camión}%
      {--Patente : String\\ --Año : Integer\\ --MáxCarga : Double}%
      {}
    \umlclass[x=12,y=0]{Zorzal}%
      {--ID : Integer\\ --Edad : Integer\\ --Género : Integer\\ --Hábitat : String}%
      {}
\end{tikzpicture}
}
...
- Existen métodos especiales: \bld{constructores}.
    ...
    - Todo objeto se dice que es la \itt{instancia} de una clase determinada.
...
\subsubtitleB{Métodos vs Operaciones:}
